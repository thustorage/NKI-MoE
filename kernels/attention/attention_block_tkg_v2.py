...
from .attention.attention_tkg import AttnTKGConfig, attention_tkg
from .attention.attention_tkg_utils import is_fp8_e4m3
from .embeddings.rope import RoPE_sbuf
from .output_projection.o_projection_v2 import o_projection_v2
from .qkv.qkv_tkgv5 import qkv_tkg as qkv_tkg_v5
...
def attention_block_tkg(
    ...
    pos_ids_sb: Optional[nl.ndarray] = None,
    ...
    Args:
        ...
        V_cache (nl.ndarray): Value cache @ HBM.
            ...
            Block KV: [num_blocks, block_len, d_head].
        attention_mask (nl.ndarray): Attention mask. Shape is [S_ctx, B, q_heads, S_tkg]
            when pos_ids_sb is None, otherwise [S_tkg, B_attn, q_heads_attn, S_tkg]
            as the active-token mask used by in-kernel mask generation.
        pos_ids_sb (Optional[nl.ndarray]): Pre-broadcasted float32 position IDs in SBUF.
            Shape [128, B_attn * S_tkg]. When provided, attention_tkg uses it to
            generate the attention mask and skips the internal position-id load and
            stream_shuffle_broadcast path.
        ...
        sbm (Optional[BufferManager]): SBUF memory manager (otherwise auto-allocated)
    ...
    # Output: QKV [B*S_tkg, I] @ SBUF
    # where I = d_head * (q_heads + 2)
    # qkv_tkg_v5 is a Qwen3 TKG hot path for B=1, S=1, H=2048, I=1280.
    rmsnorm_X_eps = 1e-6 if rmsnorm_X_eps == None else rmsnorm_X_eps
    rmsnorm_X_eps = 1e-6 if rmsnorm_X_eps == None else rmsnorm_X_eps
    kernel_assert(n_bxs_tiles == 1, "attention_block_tkg_v2 qkv_tkg_v5 path only supports one B*S tile")
    QKV_out = qkv_tkg_v5(
        hidden=X,
        qkv_w=W_qkv,
        output_layout=QKVOutputLayout.BSD,
        # Work around: qkv_tkg_v5 does not accept bias or quantization params.
    ...
    if do_out_proj:
        kernel_output = o_projection_v2(
            attention=attn_out.reshape((d_head, B, q_heads, S_tkg)),
            weight=W_out,
            # bias=bias_out,  # Work around: ops output_projection_tkg does not accept these params
            ...
            OUT_IN_SB=out_in_sb,
            name_prefix=sbm.get_name_prefix() if sbm else "",
            sbm=sbm,
        )
    else:
...
def _validate_and_extract_config(
    ...
    pos_ids_sb: Optional[nl.ndarray],
    out_in_sb: bool,
    skip_attention: bool,
) -> Dict[str, Any]:
    """
    ...
    if is_block_kv:
        ...
        S_ctx = S_max_ctx if pos_ids_sb != None else attention_mask.shape[0]
        blk_len = 0
        kernel_assert(
    ...
    expected_mask_shape = (
        (S_tkg, B_attn, q_heads_attn, S_tkg) if pos_ids_sb != None else (S_ctx, B_attn, q_heads_attn, S_tkg)
    )
    kernel_assert(
...
def _to_sbuf(buf: nl.ndarray, sbm: BufferManager) -> nl.ndarray:
    """
    Ensure buffer is in SBUF; copy from HBM if needed.

    Args:
        buf (nl.ndarray): Input buffer (HBM or SBUF)
        sbm (BufferManager): SBUF memory manager
...
def _process_head_group(
    ...
    out_qkv = sbm.alloc_stack(shape=(d, B * n_heads * S), dtype=QKV.dtype, buffer=nl.sbuf)
    for head_idx in range(n_heads):
        psum = nl.ndarray((d, B * S), dtype=QKV.dtype, buffer=nl.psum)
        nisa.nc_transpose(psum, QKV[:, nl.ds(qkv_offset + head_idx * d, d)])
        nisa.tensor_copy(out_qkv.reshape((d, B, n_heads, S))[:, :, head_idx, :], psum.reshape((d, B, S)))
    
    out = sbm.alloc_stack(shape=(d, B * n_heads * S), dtype=nl.bfloat16, buffer=nl.sbuf)

    # Pre-RoPE RMSNorm
    if rmsnorm_pre_enabled:
        _rms_norm(out, out_qkv, rmsnorm_pre_eps, w=rmsnorm_pre_W, sbm=sbm)

    # RoPE
    if enable_rope:
        out_4d = out.reshape((d, B, n_heads, S))
        out_rope = sbm.alloc_stack(out_4d.shape, dtype=nl.bfloat16, buffer=nl.sbuf)
    ...
    out_rms_norm = sbm.alloc_stack(shape=(d, B * n_heads * S), dtype=nl.bfloat16, buffer=nl.sbuf)

    # Post-RoPE RMSNorm
    if rmsnorm_post_enabled:
        _rms_norm(out_rms_norm, out, rmsnorm_post_eps, rmsnorm_post_W, sbm)
        out = out_rms_norm
        assert False
    return out
...
def _QKV_processing(
    ...
    io_dtype,
    sbm: BufferManager,
    V_hbm_out: Optional[nl.ndarray] = None,
    ...
    for tile_idx in range(n_bxs_tiles):
        ...
        if QKV.buffer != nl.sbuf:
            assert False, "HBM QKV input is not supported when B*S_tkg > pmax (multiple tiles), got B*S_tkg = {B*S_tkg}, pmax = {nl.tile_size.pmax}"
        else:
            qkv_sb = QKV
...
def _rms_norm(
    out: nl.ndarray, x: nl.ndarray, eps: float, w: Optional[nl.ndarray] = None, sbm: Optional[BufferManager] = None
) -> None:
    """
    ...
    nisa.tensor_copy(dst=out, src=out_sb)
...
def _quantize_to_fp8(tensor, scale, sbm):
    ...
    Args:
        ...
        sbm: BufferManager for allocations
