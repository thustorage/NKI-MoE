...
def rmsnorm_tkg(
    ...
    Args:
        ...
        force_no_shard (bool): If True, every core computes the full BxS range and skips local collective exchange.
    ...
    if output_view.is_sbuf():
        ...
        output_sb = alloc_tensor((H0, BxS, H1), dtype=input_view.dtype, buffer=nl.sbuf, name=f"{name_prefix}rmsnorm_output_sb")
        output_sb_view = TensorView(output_sb)

    _, lnc, shard_id = get_verified_program_sharding_info("rmsnorm_tkg", (0, 1))

    num_shards = lnc
    do_shard = not force_no_shard and num_shards == 2 and BxS > SHARDING_THRESHOLD and BxS % lnc == 0
    if not do_shard:
        num_shards, shard_id = 1, 0

    num_H_shards_in_output_H_dim = 1 if (hidden_dim_tp or no_h_interleave or force_no_shard) else lnc
